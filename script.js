...
const BOARD = ["opportunity","doodad","payday","opportunity","chance","charity",
  "opportunity","payday","doodad","chance","downsized","opportunity",
  "payday","market","doodad","opportunity","chance","payday","opportunity","market"];
...
const SPACE_META = {
  ...
  charity:{label:"GIVE", color:"#6B5B95", icon:"give"},
...
function newState(p, hardMode){
  ...
  return {
    ...
    charityTurns:0, won:false, cardDebt:0, hardMode:!!hardMode,
...
function iconPath(kind){
  switch(kind){
    ...
    case 'give':
      return '<path d="M0,5 C-6,0 -7,-5 -2.5,-5 C-1,-5 0,-3.5 0,-3 C0,-3.5 1,-5 2.5,-5 C7,-5 6,0 0,5 Z" fill="#12211B"/>';
    case 'cut':
...
function buildBoard(){
  ...
  const left=56, top=56, right=344, bottom=344;
  ...
  function pointAt(t){
    ...
    if(t <= side) return {x:left+t, y:top, edge:'top'};
    t -= side;
    if(t <= side) return {x:right, y:top+t, edge:'right'};
    t -= side;
    if(t <= side) return {x:right-t, y:bottom, edge:'bottom'};
    t -= side;
    return {x:left, y:bottom-t, edge:'left'};
  }

  // board face: a single clean rounded square, no clutter
  const face = document.createElementNS(svgNS,'rect');
  face.setAttribute('x',18); face.setAttribute('y',18);
  face.setAttribute('width',364); face.setAttribute('height',364);
  face.setAttribute('rx',22);
  face.setAttribute('fill','#1C1428');
  svg.appendChild(face);

  // a single thin guide path tracing the route tiles sit on, so it reads as one loop
  const guide = document.createElementNS(svgNS,'rect');
  guide.setAttribute('x',left); guide.setAttribute('y',top);
  guide.setAttribute('width',side); guide.setAttribute('height',side);
  guide.setAttribute('rx',10);
  guide.setAttribute('fill','none');
  guide.setAttribute('stroke','rgba(224,169,59,0.3)');
  guide.setAttribute('stroke-width','1.5');
  guide.setAttribute('stroke-dasharray','2 6');
  svg.appendChild(guide);

  for(let i=0;i<N;i++){
    const pos = pointAt(i*spacing);
    spaceCoords.push(pos);
    const meta = SPACE_META[BOARD[i]];
    const g = document.createElementNS(svgNS,'g');
    g.setAttribute('transform',`translate(${pos.x},${pos.y})`);
    const s = 15; // half-size of the tile square, kept uniform for a clean, even row of tiles

    const shadow = document.createElementNS(svgNS,'rect');
    shadow.setAttribute('x',-s); shadow.setAttribute('y',-s+2);
    ...
    rect.setAttribute('stroke','#12211B');
    rect.setAttribute('stroke-width','1.4');
    g.appendChild(rect);

    const topEdge = document.createElementNS(svgNS,'rect');
    topEdge.setAttribute('x',-s); topEdge.setAttribute('y',-s);
    topEdge.setAttribute('width',s*2); topEdge.setAttribute('height',6);
    topEdge.setAttribute('rx',3);
    topEdge.setAttribute('fill','rgba(255,255,255,0.25)');
    g.appendChild(topEdge);
  ...
  token.setAttribute('transform',`translate(${c0.x},${c0.y-24})`);
  token.innerHTML = `
    <ellipse cx="0" cy="10" rx="8" ry="2.6" fill="rgba(0,0,0,0.35)"/>
    <path d="M0,9 C-7,9 -8,-2 -5,-8 C-3,-11.5 3,-11.5 5,-8 C8,-2 7,9 0,9 Z" fill="#E0A93B" stroke="#12211B" stroke-width="1.2"/>
    <circle cx="0" cy="-9.5" r="3.7" fill="#F3EEDF" stroke="#12211B" stroke-width="1.2"/>
  `;
...
function moveTokenTo(i){
  ...
  token.setAttribute('transform',`translate(${c.x},${c.y-24})`);
}
...
function landOn(i){
  ...
  else if(type==='charity') showCharity();
...
function showCharity(){
  const amt = Math.round(state.salary*0.10);
  openModal({
    ribbon:"give something back", icon:"give", title:"Give 10% of your salary?",
    body:`You can donate $${amt} this turn. Totally optional, some people build giving into their budget on purpose instead of leaving it to chance.`,
    lesson:"There's no correct amount here. The point is deciding on purpose where your money goes, instead of it just leaking out randomly. Some people also get a tax break for donations, but that's a side bonus, not the main reason people do it.",
    buyLabel:`Give $${amt}`, canBuy: state.cash>=amt,
    onBuy:()=>{
      state.cash-=amt; state.charityTurns=3;
      log(`You gave $${amt}.`);
      closeModal(); endTurn(); renderAll();
    },
    onPass:()=>{ log("You kept your cash this turn."); closeModal(); endTurn(); }
  });
}
